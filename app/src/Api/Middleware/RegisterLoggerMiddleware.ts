...
const IGNORE_ROUTES = ['GET ', 'POST ', 'PUT ', 'GET /', 'GET /favicon.ico', 'GET /isAlive', 'GET /bhut/is-alive'];

const getMethod = async (req: Request) => {
    const {query, route} = req;
    let routePath = route && route.path ? '/api-partner' + route.path : req.path;
...
const registerLoggerMiddleware = async (startTime: [number, number], req: Request, res: Response, responseBody: any, isCloseRequest: boolean) => {
    ...
    loggerInternal.partnerUuid = 'null';
    ...
    if (loggerInternal.statusCode <= 201 || loggerInternal.statusCode === 304) {
